...
(define (kg2lb mass)
  (let* ([ weightoz (floor(* 16 2.2 mass))]
        [remainder (modulo weightoz 16)]
        [pounds (/ (- weightoz remainder) 16)])
    (string-append (number->string pounds) (if (= pounds 1) " pound" " pounds") " " (number->string remainder) (if (= remainder 1) " ounce" " ounces"))))
